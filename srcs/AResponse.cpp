...
AResponse::AResponse(const AResponse &src)
    : _request(src._request), _response(src._response), _server(src._server),
...
short AResponse::checkSize() const {
  ...
  if (it != _request.header_fields.end()) {
    ...
    // 	std::cout << "size: " << size << ", body_size: " <<
    // _request.message_body.length() << "\n"; 	return 400;}
  }
  return 200;
}
...
short AResponse::checkClientBodySize() const {
  ...
  if (_request.message_body.size() > max_body_size)
    return 413;
...
void AResponse::loadCommonHeaders() {
  _response.headers.insert(std::make_pair(std::string("Date"), getHttpDate()));
  _response.headers.insert(
...
std::string AResponse::addFileEntry(const std::string &name,
  ...
  std::string WS2 = "                   ";
  fileEntry << "<a href=\"" + name + "\">" + displayName + "</a>" + WP1 + date +
                   WS2 + size + "\n";
