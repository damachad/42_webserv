...
// Returns the value associated with a key Header
...
// Converts a vector<std::string> into a char **array
...
// Sets an Environment Variable into the local env
...
// Sets all the MUST environment variables in the local env
short CGI::setCGIEnv() {
...
// Reads the output of the script from the fd and creates a string with it's
// output
...
// Manages pipes and execve the script
void CGI::runScript(int *pipeIn, int *pipeOut, const std::string &scriptPath) {
  ...
  // Changes directory to allways run the script relative to this path
  if (chdir(dirName.c_str()) < 0)
    exit(1);

  char *argv[] = {const_cast<char *>(scriptPath.c_str()), NULL};
  if (execve(scriptPath.c_str(), argv, _cgiEnv) == -1)
    exit(1);
}

// Overall function to open pipes, run the script and retrieve it's output,
// returning it.
std::string CGI::executeCGI(const std::string &scriptPath) {
  ...
  if (pid == 0) {
    runScript(pipeIn, pipeOut, scriptPath);
  } else {
    close(pipeIn[0]);
    close(pipeOut[1]);
...
// A parser to retreive all the CGI headers into a multimap<string, string>
...
// The main motor that makes sure the response is correctly formated and
